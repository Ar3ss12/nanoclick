...
/// Check current process elevation status and always-run-as-admin configuration.
#[tauri::command]
pub fn check_elevation() -> serde_json::Value {
    serde_json::json!({
...
/// Request UAC elevation and restart the application as administrator.
#[tauri::command]
pub fn restart_as_admin() -> Result<(), String> {
    crate::platform::restart_as_admin()
}

/// Toggle Windows AppCompat registry to always run this executable as administrator.
#[tauri::command]
pub fn set_always_run_as_admin(enabled: bool) -> Result<(), String> {
    crate::platform::set_always_run_as_admin(enabled)
}

/// Query whether the executable is configured to always run as administrator.
#[tauri::command]
...
/// since launch: if it stays at 0 while the user types, the guard checkbox
/// is off or the hook is not receiving keys. `hotkeys_locked` proves the
/// toggle is currently being suppressed because the user is still typing.
#[tauri::command]
pub fn get_smart_guard_status(state: State<'_, crate::AppState>) -> serde_json::Value {
    let guard = state.scheduler.typing_guard();
    let filter = state.scheduler.app_filter();
    serde_json::json!({
        "typing_enabled": guard.is_enabled(),
        "typing_pause_ms": guard.pause_ms(),
        "hotkeys_locked": guard.hotkeys_locked(),
        "lock_remaining_ms": guard.lock_remaining_ms(),
        "text_events": guard.text_events(),
        "app_filter_mode": filter.mode().as_config_str(),
        "app_filter_active": filter.is_active(),
        "app_filter_count": filter.list().len(),
    })
}

/// Smart Guard — invariant defaults the UI needs to render its controls
/// (kept in Rust so the freeze window has a single source of truth).
#[tauri::command]
pub fn get_smart_guard_defaults() -> serde_json::Value {
    serde_json::json!({
        "typing_freeze_ms": crate::guard::default_typing_freeze_ms(),
        "app_filter_modes": ["everywhere", "whitelist", "blacklist"],
    })
}
